...
data_file <- "https://raw.githubusercontent.com/Tessshawwa/ProjetRshiny/main/ExtractionDPENordSud/df_nord_sud_COMBINE_COMPLET.csv"
...
# On charge directement depuis l'URL
print(paste("Chargement du fichier CSV depuis:", data_file)) 
...
if (!is.null(df) && is.data.frame(df) && nrow(df) > 0) {
  ...
  if(length(missing_cols) > 0){
    ...
    # Convertir les autres colonnes de texte en facteurs
  ...
  if(!show_load_error) {
    error_message <- paste("Le fichier CSV depuis", data_file, "est vide ou n'a pas pu être lu correctement.") 
    print(error_message)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$theme_selector, {
    ...
    if (selected_skin_name == "skin-black") {
      shinyjs::show(id = "dark_mode_patch")
    } else {
      shinyjs::hide(id = "dark_mode_patch")
    }
  ...
  output$evolution_dpe_plot <- renderPlotly({
    ...
    p <- ggplot(data_plot, aes(x = Mois, y = Effectif, color = Groupe, group = Groupe,
      ...
      scale_color_manual(values = c("59 - Nord - Existant" = "#1f77b4",
                                    "66 - Pyrénées-Orientales - Existant" = "#ff7f0e",
                                    "59 - Nord - Neuf" = "#2ca02c",
                                    "66 - Pyrénées-Orientales - Neuf" = "#d62728"),
                         name = "Groupe") +
